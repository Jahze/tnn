...
public:
  GAN(std::size_t msPerFrame,
    ...
    generator_.reset(new NeuralNet(2, dimension * dimension, 1, 300));
...
protected:
  ...
  void TrainModel() {
    auto TrainFunction = [this]() {
      ...
      //for (std::size_t i = 0u; i < 1000; ++i, ++progress) {
        ...
        auto lossFunction = [](double value, std::size_t idx) {
          return -(1.0 - value);
        };
        ...
        generatorInputs.push_back(RandomDouble(0.0, 1.0));
        generatorInputs.push_back(RandomDouble(0.0, 1.0));
